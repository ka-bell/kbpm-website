...
export const Route = createFileRoute("/playground/$slug")({
  loader: ({ params }) => {
    const post = getPlaygroundPost(params.slug);
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        {
          title: post ? `${post.title} — Playground` : "Playground — kbell + postman",
        },
        {
          name: "description",
          content: post?.summary ?? "Notes from the KB+PM playground.",
        },
        {
          property: "og:title",
          content: post ? `${post.title} — Playground` : "Playground",
        },
        { property: "og:description", content: post?.summary ?? "" },
      ],
      links: post
        ? [{ rel: "canonical", href: `/playground/${post.slug}` }]
        : [],
    };
  },
  component: PlaygroundPost,
});

function PlaygroundPost() {
