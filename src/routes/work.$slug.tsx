  ...
  return (
    <section className="mx-auto max-w-[1440px] px-6 py-32 md:px-8 md:py-40">
      ...
      <h1
        ...
        style={{ fontSize: "clamp(2.5rem, 6vw, 5rem)", letterSpacing: "-0.04em", lineHeight: 0.95, fontWeight: 500 }}
      >
        That case has moved.
      </h1>
      <Link
...
function getCaseDetailCards(c: CaseStudy): { problem: NarrativeCard[]; build: NarrativeCard[] } {
  const cards: Record<string, { problem: NarrativeCard[]; build: NarrativeCard[] }> = {
    spilnews: {
      problem: [
        { title: "Workflow", body: "Editorial teams needed video management, metadata, scheduling, and permissions in one publishing flow." },
        { title: "Constraint", body: "The system had to fit the SPIL ecosystem while still feeling purpose-built for video-first news." },
        { title: "Measure", body: "A launched MVP that supports modern editorial workflows instead of article-first publishing habits." },
      ],
      build: [
        { title: "Product strategy", body: "Shape the platform around video management, publishing workflow, metadata, and editorial roles." },
        { title: "Interface system", body: "Design dashboard, CMS, desktop, and mobile screens that remain clear in daily newsroom use." },
        { title: "Product build", body: "Combine front-end, back-end, CMS, design system, and product management into one usable MVP." },
      ],
    },
    "mix-interiors": {
      problem: [
        { title: "Reader need", body: "Readers could see inspiring interior products, but could not easily identify or explore them from the image." },
        { title: "Content gap", body: "Editorial photography and manufacturer product data lived in separate systems." },
        { title: "Measure", body: "A discovery experience that connects images to products and sends readers directly to manufacturers." },
      ],
      build: [
        { title: "Visual discovery", body: "Turn interior imagery into interactive product surfaces with hotspots and product context." },
        { title: "Data layer", body: "Aggregate manufacturer catalogues into a structured product database with synchronisation." },
        { title: "Matching system", body: "Support product recognition, recommendations, API integrations, and direct manufacturer links." },
      ],
    },
    "virtue-worldwide": {
      problem: [
        { title: "Strategy need", body: "Cultural strategists needed to see patterns between topics, people, organisations, and communities." },
        { title: "Constraint", body: "Traditional research was too slow and often missed hidden relationships shaping behaviour." },
        { title: "Measure", body: "A tool that makes network intelligence useful for strategy and creative teams." },
      ],
      build: [
        { title: "Knowledge graph", body: "Map relationships between topics, people, organisations, and audience communities." },
        { title: "Data visualisation", body: "Make complex network data explorable through interactive graphs and clear interface states." },
        { title: "Strategic output", body: "Support trend discovery, community detection, relationship mapping, and global brand reports." },
      ],
    },
    hopplay: {
      problem: [
        { title: "Visitor need", body: "Destinations wanted experiences that go beyond static guides and traditional walking tours." },
        { title: "Operational need", body: "Cities needed a repeatable way to create routes, challenges, and location-based content." },
        { title: "Measure", body: "Live destination experiences that can scale across multiple cities." },
      ],
      build: [
        { title: "Route builder", body: "Create curated city routes with location-based moments, challenges, and unlockable experiences." },
        { title: "Gameplay system", body: "Use maps, GPS, challenges, and gamification to make exploration feel active." },
        { title: "Multi-city CMS", body: "Give tourism teams a platform to manage content, destinations, and future implementations." },
      ],
    },
    academion: {
      problem: [
        { title: "Operations", body: "Accreditation work was spread across email threads, documents, shared drives, and manual follow-ups." },
        { title: "Constraint", body: "The platform had to respect complex review roles, evidence trails, and institutional accountability." },
        { title: "Measure", body: "Less time lost to coordination, with a clear audit trail for every review cycle." },
      ],
      build: [
        { title: "Workflow model", body: "Model submissions, reviewers, evidence, sign-offs, ownership, and states as one system." },
        { title: "Permissions", body: "Design roles and delegated access flexible enough for university review processes." },
        { title: "Audit trail", body: "Keep every artefact traceable so leadership, reviewers, and teams can trust the process." },
      ],
    },
  };
            ...
            <div className="mb-10 grid grid-cols-1 gap-6 md:mb-14 md:grid-cols-12 md:items-end">
              <div className="md:col-span-7">
                ...
                <h2
                  ...
                  style={{ fontSize: "clamp(2rem, 4vw, 3.5rem)", letterSpacing: "-0.04em", lineHeight: 0.95, fontWeight: 500 }}
                >
                  The system in use.
                </h2>
              </div>
        ...
        <div className="mx-auto max-w-[1440px] px-6 py-20 md:px-8 md:py-28">
          ...
          <div className="mt-14 grid grid-cols-1 border border-white/20 md:grid-cols-3">
            {c.results.map((r, index) => (
              <div
                key={r.label}
                className={`min-h-[280px] p-6 md:p-8 ${index < c.results.length - 1 ? "border-b border-white/20 md:border-b-0 md:border-r" : ""}`}
              >
            ...
            <div className="grid grid-cols-1 items-end gap-10 md:grid-cols-12 md:gap-10">
              ...
              <div className="md:col-span-4 md:justify-self-end">
                <Link
                  ...
                  className="group inline-flex items-center gap-4 rounded-full bg-foreground px-6 py-4 text-sm font-medium text-background hover:-translate-y-px transition-transform"
                >
                  See the service
          ...
          {related.map((r, i) => (
            <Reveal key={r.slug} delay={i * 80}>
              <Link to="/work/$slug" params={{ slug: r.slug }} className="group block">
                ...
                <h3
                  className="font-display mt-5 text-foreground"
                  style={{ fontSize: "clamp(1.25rem, 1.6vw, 1.5rem)", letterSpacing: "-0.03em", fontWeight: 500 }}
                >
  ...
  return (
    <Reveal>
      <div className="border-t border-border pt-10 md:pt-14">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-12 md:gap-12 md:items-start">
        ...
        <div className="mt-12 grid grid-cols-1 border border-border md:grid-cols-3">
          {cards.map((card, index) => (
            <div
              key={card.title}
              className={`min-h-[220px] p-6 md:p-7 ${index < cards.length - 1 ? "border-b border-border md:border-b-0 md:border-r" : ""}`}
            >
              <p className="font-mono-label uppercase tracking-wider text-muted-foreground">
